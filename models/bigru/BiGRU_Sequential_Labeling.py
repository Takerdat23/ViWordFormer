...
class OutputHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.span_classifier = nn.Linear(2*config.d_model, config.output_dim) 
...
class BiGRU_Sequential_Labeling(nn.Module):
    def __init__(self, config, vocab: Vocab):
        ...
        self.lstm = nn.GRU(config.input_dim, self.d_model, self.layer_dim, batch_first=True, dropout=config.dropout if self.layer_dim > 1 else 0, bidirectional=True)
    ...
    def forward(self, x: torch.Tensor, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass of the GRU sequence labeling model with BPE token handling.
