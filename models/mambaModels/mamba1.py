...
class MambaBlock(nn.Module):
    def __init__(self, config):
        ...
        self.d_inner = self.config.expand_factor * self.config.d_model
        self.in_proj = nn.Linear(config.d_model, 2 * self.d_inner, bias=config.bias)
        ...
        self.x_proj = nn.Linear(self.d_inner, config.dt_rank + 2 * config.d_state, bias=False)
    ...
    def selective_scan_seq(self, x, delta, A, B, C, D):
        ...
        h = torch.zeros(x.size(0), self.self.d_inner, self.config.d_state, device=deltaA.device) # (B, ED, N)
        hs = []
