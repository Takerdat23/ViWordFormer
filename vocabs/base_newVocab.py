...
class NewVocab(object):
    ...
    def is_vietnamese_word(self, word: str) -> bool:
        ...
        EXCEPTION_VIETNAMESE = ["thuyền", "nguyên", "chuyến", "khuyến", "xuyên",
                                "nguyện", "huyện", "chuyển", "uyển", "quyên", "truyện", "xuyên", "duyên", "huyền"]
        ...
        if any(char in self.english_charactor for char in word):
            return False
        ...
        vietnamese_pattern = re.compile(
            r'^[a-zA-ZàáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđĐ]+$')
        if not vietnamese_pattern.match(word):
            return False
        ...
        has_consonant = any(decomposed_word.endswith(consonant)
                            for consonant in self.FINAL_CONSONANTS)
        has_vowels_at_the_end = False
        for char in decomposed_word[::-1]:
            if char.isalpha():
                if char in self.vowels:
                    has_vowels_at_the_end = True
                    break
                else:
                    has_vowels_at_the_end = False
                    break
        if not (has_consonant or has_vowels_at_the_end):
            return False

        vowels_positions = [i for i, char in enumerate(
            decomposed_word) if char in self.vowels]
    ...
    def split_rhyme(self, rhyme: str):
        ...
        for consonant in sorted(self.FINAL_CONSONANTS, key=len, reverse=True):
            if rhyme.endswith(consonant):
                am_cuoi = consonant
                # Remove final consonant from rhyme
                rhyme = rhyme[:-len(consonant)]
                break
