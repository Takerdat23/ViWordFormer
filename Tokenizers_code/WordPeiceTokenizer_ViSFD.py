...
class WordPieceTokenizer_ViSFD(object):
    ...
    def make_vocab(self, config):
        ...
        self.i2a = {i: aspect for i, aspect in enumerate(aspects)}
        self.a2i = {aspect: i for i, aspect in enumerate(aspects)}

        sentiments = list(sentiments)
        self.i2s = {i: sentiment for i, sentiment in enumerate(sentiments, 1)}
        self.s2i = {sentiment: i for i, sentiment in enumerate(sentiments, 1)}
        self.i2s[0] = None
        self.s2i[None] = 0
    ...
    def encode_sentence(self, text, max_len=None, pad_token_id=0):
        if not self.tokenizer:
            raise ValueError(
                "Tokenizer model is not loaded. Call load_model() first.")
    ...
    def total_labels(self) -> dict:
        return {
            "aspects": len(self.a2i),
            "sentiments": len(self.s2i)
        }
